...
var players = {
  1: utils.playerCreator(
    "Server",
    "IN-PROGRESS",
    5,
    6969,
    "Welcome to the S0urce.io Private Server 0.1 Alpha!",
    69
  ),
};
    ...
    switch (data.task) {
      ...
      case 103:
        console.log(
          "player with id " +
            socket.id +
            " is trying to buy something with id " +
            data.id
        );
        break;
      case 102:
        ...
        console.log(
          "something shall be upgraded (thing " + data.id + ")at port " + port
        );
        break;
...
server.listen(config.network.port, config.network.hostname, function () {
  console.log(
    `Server listening at http://${config.network.hostname}:${config.network.port}/`
  );
});
